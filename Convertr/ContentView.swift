...
struct ContentView: View {
                    ...
                    provider.loadItem(forTypeIdentifier: UTType.fileURL.identifier, options: nil) { (urlData, error) in
                        ...
                        if let urlData = urlData as? Data,
                           let urlString = String(data: urlData, encoding: .utf8) {
                            ...
                            Task {
                                if let item = await loadDocumentAsItem(from: url) {
                ...
                provider.loadItem(forTypeIdentifier: UTType.plainText.identifier, options: nil) { (textData, error) in
                    ...
                    if let urlData = textData as? Data,
                       let urlString = String(data: urlData, encoding: .utf8) {
                        ...
                        Task {
                            if let item = await self.loadDocumentAsItem(from: url) {
                                print("Successfully loaded document from URL")
                                DispatchQueue.main.async {
                                    newItems.append(item)
                                }
                            } else {
                        ...
                        Task {
                            if let image = await self.renderTextAsImage(text) {
                                print("Successfully rendered text as image")
                                DispatchQueue.main.async {
                                    let item = ImageItem(image: image, originalURL: nil)
                                    newItems.append(item)
                                }
                            }
                            loadedItemCount += 1
                            ...
                            for i in 0..<pdfDoc.pageCount {
                                if let page = pdfDoc.page(at: i),
                                   let pageImage = await self.renderPDFPage(page) {
                                    let item = ImageItem(image: pageImage, originalURL: nil)
                                    newItems.append(item)
                                }
                            }
                            group.leave()
    ...
    private func loadDocumentAsItem(from url: URL) async -> ImageItem? {
        print("Attempting to load document from URL:", url)
        ...
        if format?.utType == .pdf {
            ...
            if let pdfDoc = PDFDocument(url: url),
               ...
               let pageImage = await renderPDFPage(page) {
                print("Successfully loaded PDF page")
                return ImageItem(
                    image: pageImage,
                    originalURL: url,
                    status: "Pending"
        ...
        } else if format?.utType == .rtf || format?.utType == .plainText {
            print("Loading text document")
            do {
                // Try to read the text content
                let text = try String(contentsOf: url, encoding: .utf8)
                print("Successfully read text content, length:", text.count)
                ...
                print("Error reading text document:", error)
                ...
                for encoding in encodings {
                    if let text = try? String(contentsOf: url, encoding: encoding) {
                        print("Successfully read text with encoding:", encoding)
                        if let image = await renderTextAsImage(text) {
                            print("Successfully rendered text as image")
                            return ImageItem(
                                image: image,
    ...
    private func pasteImages() {
        ...
        print("All pasteboard types:", pasteboard.types ?? [])
        
        // Try reading files from Finder (multiple approaches)
        if newImages.isEmpty {
            // Approach 1: Direct file URLs
            if let fileURLs = pasteboard.readObjects(forClasses: [NSURL.self], options: nil) as? [URL] {
                print("Found \(fileURLs.count) file URLs")
                for url in fileURLs {
                    if let image = NSImage(contentsOf: url) {
                        newImages.append(ImageItem(image: image, originalURL: url))
                    }
                }
            }
            
            // Approach 2: FileURL data
            if newImages.isEmpty, let urlData = pasteboard.data(forType: .fileURL) {
            ...
            // Approach 3: File promises
            if newImages.isEmpty {
                let options = [NSPasteboard.ReadingOptionKey.urlReadingContentsConformToTypes: [UTType.image.identifier]]
                if let items = pasteboard.readObjects(forClasses: [NSFilePromiseReceiver.self], options: options) {
                    print("Found \(items.count) items from promises")
        ...
        if newImages.isEmpty {
            ...
            if let images = pasteboard.readObjects(forClasses: [NSImage.self], options: nil) as? [NSImage] {
                ...
                for image in images {
                    newImages.append(ImageItem(image: image, originalURL: nil))
                }
            }
        ...
        // Add all new images to the grid
        if !newImages.isEmpty {
            print("Adding \(newImages.count) new images")
            DispatchQueue.main.async {
                self.fileTypeState.addItems(newImages, type: windowId)
            }
        } else {
    ...
    private func loadDocument(from url: URL) async {
        ...
        if format?.utType == .pdf {
            ...
            if let pdfDoc = PDFDocument(url: url) {
                for i in 0..<pdfDoc.pageCount {
                    if let page = pdfDoc.page(at: i),
                       let pageImage = await renderPDFPage(page) {
                        DispatchQueue.main.async {
                            self.fileTypeState.addItems([ImageItem(
                                image: pageImage,
                                originalURL: url,
                                status: "Pending"
                            )], type: windowId)
            ...
            // Handle text documents by rendering them
            if let image = await renderTextDocument(from: url) {
                DispatchQueue.main.async {
                    self.fileTypeState.addItems([ImageItem(
                        image: image,
                        originalURL: url,
    ...
    private func renderTextDocument(from url: URL) async -> NSImage? {
        do {
            let text = try String(contentsOf: url, encoding: .utf8)
            let attributedString = NSAttributedString(
                string: text,
            ...
            let textContainer = NSTextContainer(size: CGSize(width: 600, height: CGFloat.greatestFiniteMagnitude))
            layoutManager.addTextContainer(textContainer)
            ...
            print("Error rendering text document:", error)
            return nil
        }
    }
}
